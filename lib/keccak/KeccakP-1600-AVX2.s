...
# -----------------------------------------------------------------------------
#
# void KeccakAVX2_Initialize(void *state);
#
.ifdef macOS
.globl  _KeccakAVX2_Initialize
_KeccakAVX2_Initialize:
.else
.globl  KeccakAVX2_Initialize
.type   KeccakAVX2_Initialize,@function
KeccakAVX2_Initialize:
.endif
.balign 32
...
.ifdef macOS
.else
.size   KeccakAVX2_Initialize,.-KeccakAVX2_Initialize
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_AddByte(void *state, unsigned char data, unsigned int offset);
#                                %rdi                 %rsi               %rdx
#
.ifdef macOS
.globl  _KeccakAVX2_AddByte
_KeccakAVX2_AddByte:
.else
.globl  KeccakAVX2_AddByte
.type   KeccakAVX2_AddByte,@function
KeccakAVX2_AddByte:
.endif
.balign 32
    mov         %rdx, %rax
...
.ifdef macOS
.else
.size   KeccakAVX2_AddByte,.-KeccakAVX2_AddByte
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_AddBytes(void *state, const unsigned char *data, unsigned int offset, unsigned int length);
#                                %rdi                         %rsi               %rdx                 %rcx
#
.ifdef macOS
.globl  _KeccakAVX2_AddBytes
_KeccakAVX2_AddBytes:
.else
.globl  KeccakAVX2_AddBytes
.type   KeccakAVX2_AddBytes,@function
KeccakAVX2_AddBytes:
.endif
.balign 32
    cmp         $0, %rcx
    jz          KeccakAVX2_AddBytes_Exit
    mov         %rdx, %rax                              # rax offset in lane
    and         $0xFFFFFFF8, %edx                       # rdx pointer into state index mapper
    lea         mapState(%rip), %r9
    add         %r9, %rdx
    and         $7, %rax
    jz          KeccakAVX2_AddBytes_LaneAlignedCheck
    ...
    add         $8, %rdx
    add         %rdi, %rax
KeccakAVX2_AddBytes_NotAlignedLoop:
    mov         (%rsi), %r8b
    inc         %rsi
    xorb        %r8b, (%rax)
    inc         %rax
    dec         %r9
    jnz         KeccakAVX2_AddBytes_NotAlignedLoop
    jmp         KeccakAVX2_AddBytes_LaneAlignedCheck
KeccakAVX2_AddBytes_LaneAlignedLoop:
    mov         (%rsi), %r8
    add         $8, %rsi
    mov         (%rdx), %rax
    add         $8, %rdx
    add         %rdi, %rax
    xor         %r8, (%rax)
KeccakAVX2_AddBytes_LaneAlignedCheck:
    sub         $8, %rcx
    jnc         KeccakAVX2_AddBytes_LaneAlignedLoop
KeccakAVX2_AddBytes_LastIncompleteLane:
    add         $8, %rcx
    jz          KeccakAVX2_AddBytes_Exit
    mov         (%rdx), %rax
    add         %rdi, %rax
KeccakAVX2_AddBytes_LastIncompleteLaneLoop:
    mov         (%rsi), %r8b
    inc         %rsi
    xor         %r8b, (%rax)
    inc         %rax
    dec         %rcx
    jnz         KeccakAVX2_AddBytes_LastIncompleteLaneLoop
KeccakAVX2_AddBytes_Exit:
    ret
.ifdef macOS
.else
.size   KeccakAVX2_AddBytes,.-KeccakAVX2_AddBytes
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_OverwriteBytes(void *state, const unsigned char *data, unsigned int offset, unsigned int length);
#                                       %rdi                        %rsi               %rdx                 %rcx
#
.ifdef macOS
.globl  _KeccakAVX2_OverwriteBytes
_KeccakAVX2_OverwriteBytes:
.else
.globl  KeccakAVX2_OverwriteBytes
.type   KeccakAVX2_OverwriteBytes,@function
KeccakAVX2_OverwriteBytes:
.endif
.balign 32
    cmp         $0, %rcx
    jz          KeccakAVX2_OverwriteBytes_Exit
    mov         %rdx, %rax                              # rax offset in lane
    and         $0xFFFFFFF8, %edx                       # rdx pointer into state index mapper
    lea         mapState(%rip), %r9
    add         %r9, %rdx
    and         $7, %rax
    jz          KeccakAVX2_OverwriteBytes_LaneAlignedCheck
    mov         $8, %r9                                 # r9 is (max) length of incomplete lane
    sub         %rax, %r9
    cmp         %rcx, %r9
    cmovae      %rcx, %r9
    ...
    add         $8, %rdx
    add         %rdi, %rax
KeccakAVX2_OverwriteBytes_NotAlignedLoop:
    mov         (%rsi), %r8b
    inc         %rsi
    mov         %r8b, (%rax)
    inc         %rax
    dec         %r9
    jnz         KeccakAVX2_OverwriteBytes_NotAlignedLoop
    jmp         KeccakAVX2_OverwriteBytes_LaneAlignedCheck
KeccakAVX2_OverwriteBytes_LaneAlignedLoop:
    mov         (%rsi), %r8
    add         $8, %rsi
    mov         (%rdx), %rax
    add         $8, %rdx
    add         %rdi, %rax
    mov         %r8, (%rax)
KeccakAVX2_OverwriteBytes_LaneAlignedCheck:
    sub         $8, %rcx
    jnc         KeccakAVX2_OverwriteBytes_LaneAlignedLoop
KeccakAVX2_OverwriteBytes_LastIncompleteLane:
    add         $8, %rcx
    jz          KeccakAVX2_OverwriteBytes_Exit
    mov         (%rdx), %rax
    add         %rdi, %rax
KeccakAVX2_OverwriteBytes_LastIncompleteLaneLoop:
    mov         (%rsi), %r8b
    inc         %rsi
    mov         %r8b, (%rax)
    inc         %rax
    dec         %rcx
    jnz         KeccakAVX2_OverwriteBytes_LastIncompleteLaneLoop
KeccakAVX2_OverwriteBytes_Exit:
    ret
.ifdef macOS
.else
.size   KeccakAVX2_OverwriteBytes,.-KeccakAVX2_OverwriteBytes
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_OverwriteWithZeroes(void *state, unsigned int byteCount);
#                                            %rdi                %rsi
#
.ifdef macOS
.globl  _KeccakAVX2_OverwriteWithZeroes
_KeccakAVX2_OverwriteWithZeroes:
.else
.globl  KeccakAVX2_OverwriteWithZeroes
.type   KeccakAVX2_OverwriteWithZeroes,@function
KeccakAVX2_OverwriteWithZeroes:
.endif
.balign 32
    cmp         $0, %rsi
    jz          KeccakAVX2_OverwriteWithZeroes_Exit
    lea         mapState(%rip), %rdx                          # rdx pointer into state index mapper
    jmp         KeccakAVX2_OverwriteWithZeroes_LaneAlignedCheck
KeccakAVX2_OverwriteWithZeroes_LaneAlignedLoop:
    mov         (%rdx), %rax
    add         $8, %rdx
    add         %rdi, %rax
    movq        $0, (%rax)
KeccakAVX2_OverwriteWithZeroes_LaneAlignedCheck:
    sub         $8, %rsi
    jnc         KeccakAVX2_OverwriteWithZeroes_LaneAlignedLoop
KeccakAVX2_OverwriteWithZeroes_LastIncompleteLane:
    add         $8, %rsi
    jz          KeccakAVX2_OverwriteWithZeroes_Exit
    mov         (%rdx), %rax
    add         %rdi, %rax
KeccakAVX2_OverwriteWithZeroes_LastIncompleteLaneLoop:
    movb        $0, (%rax)
    inc         %rax
    dec         %rsi
    jnz         KeccakAVX2_OverwriteWithZeroes_LastIncompleteLaneLoop
KeccakAVX2_OverwriteWithZeroes_Exit:
    ret
.ifdef macOS
.else
.size   KeccakAVX2_OverwriteWithZeroes,.-KeccakAVX2_OverwriteWithZeroes
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_ExtractBytes(const void *state, unsigned char *data, unsigned int offset, unsigned int length);
#                                           %rdi                  %rsi               %rdx                 %rcx
#
.ifdef macOS
.globl  _KeccakAVX2_ExtractBytes
_KeccakAVX2_ExtractBytes:
.else
.globl  KeccakAVX2_ExtractBytes
.type   KeccakAVX2_ExtractBytes,@function
KeccakAVX2_ExtractBytes:
.endif
.balign 32
    push        %rbx
    cmp         $0, %rcx
    jz          KeccakAVX2_ExtractBytes_Exit
    mov         %rdx, %rax                              # rax offset in lane
    and         $0xFFFFFFF8, %edx                       # rdx pointer into state index mapper
    lea         mapState(%rip), %r9
    add         %r9, %rdx
    and         $7, %rax
    jz          KeccakAVX2_ExtractBytes_LaneAlignedCheck
    ...
    add         %rdi, %r9
    add         %rax, %r9
KeccakAVX2_ExtractBytes_NotAlignedLoop:
    mov         (%r9), %r8b
    inc         %r9
    mov         %r8b, (%rsi)
    inc         %rsi
    dec         %rbx
    jnz         KeccakAVX2_ExtractBytes_NotAlignedLoop
    jmp         KeccakAVX2_ExtractBytes_LaneAlignedCheck
KeccakAVX2_ExtractBytes_LaneAlignedLoop:
    mov         (%rdx), %rax
    add         $8, %rdx
    add         %rdi, %rax
    mov         (%rax), %r8
    mov         %r8, (%rsi)
    add         $8, %rsi
KeccakAVX2_ExtractBytes_LaneAlignedCheck:
    sub         $8, %rcx
    jnc         KeccakAVX2_ExtractBytes_LaneAlignedLoop
KeccakAVX2_ExtractBytes_LastIncompleteLane:
    add         $8, %rcx
    jz          KeccakAVX2_ExtractBytes_Exit
    mov         (%rdx), %rax
    add         %rdi, %rax
    mov         (%rax), %r8
KeccakAVX2_ExtractBytes_LastIncompleteLaneLoop:
    mov         %r8b, (%rsi)
    shr         $8, %r8
    inc         %rsi
    dec         %rcx
    jnz         KeccakAVX2_ExtractBytes_LastIncompleteLaneLoop
KeccakAVX2_ExtractBytes_Exit:
    pop         %rbx
    ret
.ifdef macOS
.else
.size   KeccakAVX2_ExtractBytes,.-KeccakAVX2_ExtractBytes
.endif

# -----------------------------------------------------------------------------
#
# void KeccakAVX2_ExtractAndAddBytes(const void *state, const unsigned char *input, unsigned char *output, unsigned int offset, unsigned int length);
#                                                 %rdi                        %rsi                  %rdx                 %rcx                  %r8
#
.ifdef macOS
.globl  _KeccakAVX2_ExtractAndAddBytes
_KeccakAVX2_ExtractAndAddBytes:
.else
.globl  KeccakAVX2_ExtractAndAddBytes
.type   KeccakAVX2_ExtractAndAddBytes,@function
KeccakAVX2_ExtractAndAddBytes:
.endif
.balign 32
    push        %rbx
    push        %r10
    cmp         $0, %r8
    jz          KeccakAVX2_ExtractAndAddBytes_Exit
    mov         %rcx, %rax                              # rax offset in lane
    and         $0xFFFFFFF8, %ecx                       # rcx pointer into state index mapper
    lea         mapState(%rip), %r9
    add         %r9, %rcx
    and         $7, %rax
    jz          KeccakAVX2_ExtractAndAddBytes_LaneAlignedCheck
    mov         $8, %rbx                                # rbx is (max) length of incomplete lane
    sub         %rax, %rbx
    cmp         %r8, %rbx
    cmovae      %r8, %rbx
    ...
    add         %rdi, %r9
    add         %rax, %r9
KeccakAVX2_ExtractAndAddBytes_NotAlignedLoop:
    mov         (%r9), %r10b
    inc         %r9
    xor         (%rsi), %r10b
    inc         %rsi
    ...
    inc         %rdx
    dec         %rbx
    jnz         KeccakAVX2_ExtractAndAddBytes_NotAlignedLoop
    jmp         KeccakAVX2_ExtractAndAddBytes_LaneAlignedCheck
KeccakAVX2_ExtractAndAddBytes_LaneAlignedLoop:
    mov         (%rcx), %rax
    add         $8, %rcx
    add         %rdi, %rax
    mov         (%rax), %r10
    ...
    mov         %r10, (%rdx)
    add         $8, %rdx
KeccakAVX2_ExtractAndAddBytes_LaneAlignedCheck:
    sub         $8, %r8
    jnc         KeccakAVX2_ExtractAndAddBytes_LaneAlignedLoop
KeccakAVX2_ExtractAndAddBytes_LastIncompleteLane:
    add         $8, %r8
    jz          KeccakAVX2_ExtractAndAddBytes_Exit
    mov         (%rcx), %rax
    add         %rdi, %rax
    mov         (%rax), %r10
KeccakAVX2_ExtractAndAddBytes_LastIncompleteLaneLoop:
    xor         (%rsi), %r10b
    inc         %rsi
    mov         %r10b, (%rdx)
    inc         %rdx
    shr         $8, %r10
    dec         %r8
    jnz         KeccakAVX2_ExtractAndAddBytes_LastIncompleteLaneLoop
KeccakAVX2_ExtractAndAddBytes_Exit:
    pop         %r10
    pop         %rbx
    ret
.ifdef macOS
.else
.size   KeccakAVX2_ExtractAndAddBytes,.-KeccakAVX2_ExtractAndAddBytes
.endif
...
.ifdef macOS
.globl  _KeccakAVX2_Permute_24rounds
_KeccakAVX2_Permute_24rounds:
.else
.globl  KeccakAVX2_Permute_24rounds
.type   KeccakAVX2_Permute_24rounds,@function
KeccakAVX2_Permute_24rounds:
.endif
.balign 32
...
.ifdef macOS
.else
.size   KeccakAVX2_Permute_24rounds,.-KeccakAVX2_Permute_24rounds
.endif

.ifdef macOS
.globl  _KeccakAVX2_Permute_12rounds
_KeccakAVX2_Permute_12rounds:
.else
.globl  KeccakAVX2_Permute_12rounds
.type   KeccakAVX2_Permute_12rounds,@function
KeccakAVX2_Permute_12rounds:
.endif
.balign 32
    lea             rhotates_left+96(%rip),%r8
    lea             rhotates_right+96(%rip),%r9
...
.ifdef macOS
.else
.size   KeccakAVX2_Permute_12rounds,.-KeccakAVX2_Permute_12rounds
.endif

.ifdef macOS
.globl  _KeccakAVX2_Permute_Nrounds
_KeccakAVX2_Permute_Nrounds:
.else
.globl  KeccakAVX2_Permute_Nrounds
.type   KeccakAVX2_Permute_Nrounds,@function
KeccakAVX2_Permute_Nrounds:
.endif
.balign 32
    lea             rhotates_left+96(%rip),%r8
    lea             rhotates_right+96(%rip),%r9
...
.ifdef macOS
.else
.size   KeccakAVX2_Permute_Nrounds,.-KeccakAVX2_Permute_Nrounds
.endif
...
KeccakF1600_FastLoop_Absorb_LanesAddLoop:
    ...
    push            %rcx
    .ifdef macOS
    call            _KeccakAVX2_Permute_24rounds
    .else
    call            KeccakAVX2_Permute_24rounds@PLT
    .endif
    pop             %rcx
    pop             %rdx
    pop             %rsi
...
# -----------------------------------------------------------------------------
#
# size_t KeccakAVX2_12rounds_FastLoop_Absorb(void *state, unsigned int laneCount, const unsigned char *data, size_t dataByteLen);
#                                          %rdi                %rsi                            %rdx         %rcx
#
.ifdef macOS
.globl  _KeccakAVX2_12rounds_FastLoop_Absorb
_KeccakAVX2_12rounds_FastLoop_Absorb:
.else
.globl  KeccakAVX2_12rounds_FastLoop_Absorb
.type   KeccakAVX2_12rounds_FastLoop_Absorb,@function
KeccakAVX2_12rounds_FastLoop_Absorb:
.endif
.balign 32
    push            %rbx
    push            %r10
    ...
    mov             %rdx, %rbx              # rbx = initial data pointer
    cmp             %rsi, %rcx
    jb              KeccakAVX2_12rounds_FastLoop_Absorb_Exit
    vzeroupper
    cmp             $21, %rsi    
    jnz             KeccakAVX2_12rounds_FastLoop_Absorb_Not21Lanes
    sub             $21, %rcx
    lea             rhotates_left+96(%rip),%r8
    lea             rhotates_right+96(%rip),%r9
    lea             96(%rdi),%rdi
    ...
    vmovdqu         8+32*4-96(%rdi),%ymm5
    vmovdqu         8+32*5-96(%rdi),%ymm6
KeccakAVX2_12rounds_FastLoop_Absorb_Loop21Lanes:    
    vpbroadcastq    (%rdx),%ymm7
    vmovdqu         8(%rdx),%ymm8
    ...
    call            __KeccakF1600
    sub             $21, %rcx
    jnc             KeccakAVX2_12rounds_FastLoop_Absorb_Loop21Lanes
KeccakAVX2_12rounds_FastLoop_Absorb_SaveAndExit:
    vmovq           %xmm0,-96(%rdi)
    vmovdqu         %ymm1,8+32*0-96(%rdi)
    vmovdqu         %ymm2,8+32*1-96(%rdi)
    vmovdqu         %ymm3,8+32*2-96(%rdi)
    ...
    vmovdqu         %ymm5,8+32*4-96(%rdi)
    vmovdqu         %ymm6,8+32*5-96(%rdi)
KeccakAVX2_12rounds_FastLoop_Absorb_Exit:
    vzeroupper
    mov             %rdx, %rax               # return number of bytes processed
    sub             %rbx, %rax
    pop             %r10
    pop             %rbx
    ret
KeccakAVX2_12rounds_FastLoop_Absorb_Not21Lanes:
    cmp             $17, %rsi    
    jnz             KeccakAVX2_12rounds_FastLoop_Absorb_Not17Lanes
    sub             $17, %rcx
    lea             rhotates_left+96(%rip),%r8
    lea             rhotates_right+96(%rip),%r9
    lea             96(%rdi),%rdi
    ...
    vmovdqu         8+32*4-96(%rdi),%ymm5
    vmovdqu         8+32*5-96(%rdi),%ymm6
KeccakAVX2_12rounds_FastLoop_Absorb_Loop17Lanes:    
    vpbroadcastq    (%rdx),%ymm7
    vmovdqu         8(%rdx),%ymm8
    ...
    call            __KeccakF1600
    sub             $17, %rcx
    jnc             KeccakAVX2_12rounds_FastLoop_Absorb_Loop17Lanes
    jmp             KeccakAVX2_12rounds_FastLoop_Absorb_SaveAndExit
KeccakAVX2_12rounds_FastLoop_Absorb_Not17Lanes:
    lea             mapState(%rip), %r9
    mov             %rsi, %rax
KeccakAVX2_12rounds_FastLoop_Absorb_LanesAddLoop:
    mov             (%rdx), %r8
    add             $8, %rdx
    mov             (%r9), %r10
    add             $8, %r9
    ...
    xor             %r8, (%r10)
    sub             $1, %rax
    jnz             KeccakAVX2_12rounds_FastLoop_Absorb_LanesAddLoop
    sub             %rsi, %rcx
    push            %rdi
    push            %rsi
    push            %rdx
    push            %rcx
.ifdef macOS
    call            _KeccakAVX2_Permute_12rounds
.else
    call            KeccakAVX2_Permute_12rounds@PLT
.endif
    pop             %rcx
    pop             %rdx
    pop             %rsi
    pop             %rdi
    cmp             %rsi, %rcx
    jae             KeccakAVX2_12rounds_FastLoop_Absorb_Not17Lanes
    jmp             KeccakAVX2_12rounds_FastLoop_Absorb_Exit
.ifdef macOS
.else
.size   KeccakAVX2_12rounds_FastLoop_Absorb,.-KeccakAVX2_12rounds_FastLoop_Absorb
.endif
