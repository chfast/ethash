...
std::shared_ptr<epoch_context> current_context;

std::shared_ptr<epoch_context> get_epoch_context(int epoch_number)
{
    std::lock_guard<std::mutex> lock{cache_building_mutex};

    if (!current_context || current_context->epoch_number != epoch_number)
        current_context = {create_epoch_context(epoch_number), destroy_epoch_context};
...
bool verify(int block_number, const hash256& header_hash, const hash256& mix_hash, uint64_t nonce,
    uint64_t target)
{
    std::shared_ptr<epoch_context> epoch_context =
